...
export function ProjectPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <Seo title={`${project.title} — Leficious`} description={project.summary} path={localizedPath(`/projects/${project.slug}`)} image={project.cover} imageWidth={project.coverWidth} imageHeight={project.coverHeight} />
      <article id="main-content" tabIndex={-1} className="mx-auto max-w-5xl px-6 py-16 md:py-24">
        <Link to={localizedPath("/")} viewTransition className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:text-accent">← {text("All projects", "全部项目")}</Link>
        ...
        <nav className="mt-24 flex items-center justify-between border-t border-border/60 pt-8"><Link to={localizedPath("/")} viewTransition className="font-mono text-xs uppercase tracking-[0.2em] text-muted-foreground transition-colors hover:text-accent">← {text("All projects", "全部项目")}</Link><Link to={localizedPath(`/projects/${next.slug}`)} viewTransition className="group text-right"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{text("Next", "下一个")}</p><p className="font-display text-lg font-semibold transition-colors group-hover:text-accent">{next.title} →</p></Link></nav>
      </article>
      <SiteFooter />
    </div>
  );
