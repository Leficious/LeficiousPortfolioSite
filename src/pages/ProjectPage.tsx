...
export function ProjectPage() {
  ...
  const relatedGallery = galleryEntries.filter((entry) => project.relatedGalleryIds?.includes(entry.id));
      ...
      <article id="main-content" tabIndex={-1} className="mx-auto max-w-5xl px-6 py-16 md:py-24">
        ...
        <section className="mt-10 overflow-hidden rounded-lg border border-border bg-surface/45" aria-labelledby="project-brief-title">
          ...
          <dl className="grid md:grid-cols-3">
            {[
              ["Design goal", project.designGoal],
          ...
          <div className="border-t border-border/60 px-5 py-5 sm:px-6">
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Delivered</p>
        ...
        {relatedGallery.length > 0 && (
          <section className="mt-20 border-t border-border/60 pt-10 md:mt-28 md:pt-14" aria-labelledby="related-gallery-title">
            <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
              <div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-accent">Related gallery work</p><h2 id="related-gallery-title" className="mt-3 font-display text-2xl font-semibold">Visuals and production studies.</h2></div>
              <Link to="/gallery" viewTransition className="font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground transition-colors hover:text-accent">View gallery →</Link>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              {relatedGallery.map((entry) => (
                <Link key={entry.id} to={`/gallery?entry=${entry.id}`} viewTransition className="group overflow-hidden rounded-lg border border-border bg-surface/45 transition-colors hover:border-accent/70">
                  ...
                  <div className="p-4"><p className="font-display font-semibold transition-colors group-hover:text-accent">{entry.title}</p><p className="mt-2 font-mono text-[9px] uppercase tracking-[0.14em] text-muted-foreground">Open gallery set →</p></div>
                </Link>
              ))}
            </div>
          </section>
