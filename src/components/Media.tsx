  ...
  return (
    <figure className="space-y-2">
      <div className="overflow-hidden rounded-lg border border-border bg-surface">
        {item.type === "image" && <img src={item.src} alt={item.alt} loading="lazy" className="h-auto w-full" />}
        {item.type === "video" && <video src={item.src} poster={item.poster} controls playsInline className="h-auto w-full" />}
        {item.type === "youtube" && (
